...
public class WebSecurityConfigurerAdaptor extends WebSecurityConfigurerAdapter {
    ...
    private String[] authPath = new String[]{"/auth/**", "/admin/**", "/api/**"};
    ...
    public boolean hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (principal instanceof LoginUser) {
            ...
            for (String url : ((LoginUser) principal).getResource().values()) {
                if (url.contains(",")) {
    ...
    private AuthenticationSuccessHandler authenticationSuccessHandler() {
        ...
        return (httpServletRequest, httpServletResponse, authentication) -> {
            ...
            if (!principal.isRoot()) {
                ...
                principal.setResource(roleResourcesMap);
            }
    ...
    class LoginUserDetailsService extends JdbcDaoImpl {
        ...
        protected UserDetails createUserDetails(String username, UserDetails userFromUserQuery, List<GrantedAuthority> combinedAuthorities) {
            String name = userFromUserQuery.getUsername();
            if (!isUsernameBasedPrimaryKey()) {
                name = username;
            }
            String id = ((LoginUser) userFromUserQuery).getId();
            boolean root = ((LoginUser) userFromUserQuery).isRoot();
            return new LoginUser(name, userFromUserQuery.getPassword(), userFromUserQuery.isEnabled(), true, true, true, combinedAuthorities, id, root);
        }
    }
}
